...
$( "#gogo" ).click(async function() {
    ...
    try{
       ...
       /* setprogress("Checking metamask...")
        ...
        let host_addr = await connectWallet(); //await get_host_addr();
        ...
        let contract_addr = await deplyContract(symbol, title, uri); // await inti_contract(symbol, title, uri)
        ...
        $("#contract_addr").children("a").attr("href",`https://lisbon.tracker.solidwallet.io/contract/${contract_addr}`)
